...
- `skills/exposing-services.md` — read this whenever the user asks you to run/start/serve a file, app, or service "on port N" or asks you to make something reachable from outside. It explains the port→URL convention for this Space.
...
- If the user asks you to run, start, or serve any file/app/service "on port N" (or asks how to make something reachable from outside this Space), read `skills/exposing-services.md` first and follow it — do not guess at host/port binding or assume a manual nginx change is needed.
- `plugins/primary-port` provides `set_primary_port`, `clear_primary_port`, and `get_primary_port` tools. Use these — not a raw `set-primary-port.sh` shell call — whenever the operator asks, in any phrasing or language, to make a port "primary"/"main"/"root", to fix a login/redirect loop on an app with its own pages or client-side routing, or to "disable"/"unset"/"clear"/"turn off" the primary port. These only run `nginx -s reload`; they never restart or rebuild the Space.
